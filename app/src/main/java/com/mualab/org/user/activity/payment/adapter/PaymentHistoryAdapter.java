...
public class PaymentHistoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (!item.userProfileImage.equals("")){
            Picasso.with(context).load(item.userProfileImage).placeholder(R.drawable.defoult_user_img).
        ...
        public void onClick(View view) {
            switch (view.getId()){
                case  R.id.rlContainer:
                    final PaymentHistory item2 = paymentHistories.get(getAdapterPosition());
                    Intent intent = new Intent(context, BookingDetailActivity.class);
                    intent.putExtra("bookingId",  String.valueOf(item2._id));
                    intent.putExtra("artistName",  item2.userName);
                    intent.putExtra("artistProfile",  item2.userProfileImage);
