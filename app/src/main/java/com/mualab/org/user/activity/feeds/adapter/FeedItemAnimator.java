...
import com.mualab.org.user.activity.feeds.adapter.Utils;
...
/**
 * Created by Dharmrja Acharya on 17.10.2017.
 */
public class FeedItemAnimator extends DefaultItemAnimator {
    private static final DecelerateInterpolator DECCELERATE_INTERPOLATOR = new DecelerateInterpolator();
    private static final AccelerateInterpolator ACCELERATE_INTERPOLATOR = new AccelerateInterpolator();
    private static final OvershootInterpolator OVERSHOOT_INTERPOLATOR = new OvershootInterpolator(1);
    ...
    public boolean animateChange(@NonNull RecyclerView.ViewHolder oldHolder,
        ...
        if (preInfo instanceof FeedItemHolderInfo) {
            ...
            if (newHolder instanceof FeedAdapter.CellFeedViewHolder){
            ...
            }else if(newHolder instanceof FeedAdapter.FeedVideoHolder){
                FeedAdapter.FeedVideoHolder holder = (FeedAdapter.FeedVideoHolder) newHolder;
                animateHeartButton(holder);
                // updateLikesCounter(holder, holder.getFeedItem().likesCount);
                if (FeedAdapter.ACTION_LIKE_IMAGE_CLICKED.equals(feedItemHolderInfo.updateAction)) {
                    animatePhotoLike(holder);
                }
            }else if (newHolder instanceof FeedAdapter.FeedTextHolder){
                animateHeartButton((FeedAdapter.FeedTextHolder)newHolder);
            }
        }
    ...
    private void animateHeartButton(final FeedAdapter.FeedTextHolder holder) {

        AnimatorSet animatorSet = new AnimatorSet();
        ObjectAnimator rotation = ObjectAnimator.ofFloat(holder.btnLike, "rotation", 0.0f, 360.0f);
        rotation.setDuration(500);
        rotation.setInterpolator(OVERSHOOT_INTERPOLATOR);
        ObjectAnimator scaleX = ObjectAnimator.ofFloat(holder.btnLike, "scaleX", 1.0f, 0.6f, 1.0f);
        scaleX.setDuration(500);
        scaleX.setInterpolator(OVERSHOOT_INTERPOLATOR);
        ObjectAnimator scaleY = ObjectAnimator.ofFloat(holder.btnLike, "scaleY", 1.0f, 0.6f, 1.0f);
        scaleY.setDuration(500);
        scaleY.setInterpolator(OVERSHOOT_INTERPOLATOR);

        rotation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
               /* if (valueAnimator.getCurrentPlayTime() >= 400) {
                    holder.btnLike.setImageResource(R.drawable.liked_icon);
                }*/
            }
        });
    ...
    private void animateHeartButton(final FeedAdapter.FeedVideoHolder holder) {
    ...
    private void animatePhotoLike(final FeedAdapter.FeedVideoHolder holder) {
        holder.vBgLike.setVisibility(View.VISIBLE);
        holder.ivLike.setVisibility(View.VISIBLE);
    ...
    private void dispatchChangeFinishedIfAllAnimationsEnded(FeedAdapter.FeedVideoHolder holder) {
        if (likeAnimationsMap.containsKey(holder) || heartAnimationsMap.containsKey(holder)) {
            return;
        }
    ...
    private void resetLikeAnimationState(FeedAdapter.FeedVideoHolder holder) {
        holder.vBgLike.setVisibility(View.INVISIBLE);
        holder.ivLike.setVisibility(View.INVISIBLE);
    }
    ...
    public static class FeedItemHolderInfo extends ItemHolderInfo {
        ...
        private FeedItemHolderInfo(String updateAction) {
